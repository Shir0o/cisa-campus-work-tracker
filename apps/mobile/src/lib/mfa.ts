...
/**
 * TOTP multi-factor helpers — a copy of the web app's src/lib/mfa.ts so both
 * surfaces share one implementation. Firebase TOTP MFA is NOT exercised by the
 * Auth emulator (it only supports SMS MFA), so every function here is either
 * pure or a thin wrapper over the firebase/auth API, keeping the flow unit-
 * testable with a mocked module.
 */
