...
### Fixed
- **`packages/core/test/board.test.ts` — Removed Stray Duplicate `parseDocNotes`/`parseDocTasks`/`collectDocTaskNodes` Tests** — These three functions are TipTap-editor-specific (`collectDocTaskNodes` walks a live ProseMirror document) and only ever run in the web coordination editor; `apps/mobile`'s Board is read-only and carries no TipTap dependency, so the platform-agnostic `@cisa/core` package never exported them. Two `describe` blocks had been copy-pasted into the core test file anyway, importing the (nonexistent) functions from `../src/board` and failing with `TypeError: … is not a function`. Removed the duplicate blocks and their unused imports — equivalent coverage already exists in `src/test/board.test.ts` against the real functions in `src/lib/board.ts`.
