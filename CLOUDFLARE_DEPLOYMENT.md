...
- `functions/__/auth/[[path]].ts` — transparently forwards every `cisa-campus-work-traker.pages.dev/__/auth/*` request to `sac-campus-hub.firebaseapp.com/__/auth/*` (no redirects, no body rewrites).
...
1. **Google Cloud Console** (the OAuth 2.0 client that backs `sac-campus-hub`):
   add `https://cisa-campus-work-traker.pages.dev/__/auth/handler` to
...
2. **Deploy** the Pages project with `VITE_FIREBASE_AUTH_DOMAIN`
   `= cisa-campus-work-traker.pages.dev` in the build env (QA: `cisa-campus-work-traker-qa.pages.dev`).
