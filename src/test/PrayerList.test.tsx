...
import { __resetPrayerSessionCache } from '../lib/prayerSession';
...
describe('PrayerList — Pray together session', () => {
  const openPrayer = {
    id: 'p1',
    data: () => ({
      contactId: 'c1',
  ...
  const openPrayer2 = {
    ...
    data: () => ({
      contactId: 'c2',
      burden: 'Wisdom for the semester',
  ...
  beforeEach(() => {
    ...
    vi.mocked(onSnapshot).mockImplementation((ref: any, callback: any) => {
      if (ref?.path === 'contacts') {
        ...
        callback({ docs: [openPrayer, openPrayer2], size: 2 });
      } else if (ref?.path === 'users') {
        callback({ docs: [{ id: 'u-test', data: () => ({ displayName: 'Test User', approved: true, role: 'admin' }) }], size: 1 });
      } else {
        callback({ docs: [], size: 0 });
  ...
  it('shows a Pray together launch card when someone has an open prayer', async () => {
    render(<PrayerList />);
    await waitFor(() => expect(screen.getByText('Alice Johnson')).toBeInTheDocument());
    expect(screen.getByText(/Pray through them one at a time/i)).toBeInTheDocument();
    expect(screen.getAllByRole('button', { name: /Pray together/i }).length).toBeGreaterThan(0);
  });

  it('hides the launch card when every prayer is settled', async () => {
    const answered = [
      { id: 'p1', data: () => ({ contactId: 'c1', burden: 'Strength for finals', date: '2026-06-10T00:00:00.000Z', status: 'answered', updatedAt: '2026-06-10T00:00:00.000Z' }) },
    ];
    vi.mocked(onSnapshot).mockImplementation((ref: any, callback: any) => {
      if (ref?.path === 'contacts') {
        callback({ docs: mockContacts, size: 2 });
      } else if (ref?.path === 'prayers') {
        callback({ docs: answered, size: 1 });
      } else if (ref?.path === 'users') {
        callback({ docs: [{ id: 'u-test', data: () => ({ displayName: 'Test User', approved: true, role: 'admin' }) }], size: 1 });
      } else {
        callback({ docs: [], size: 0 });
    ...
    render(<PrayerList />);
    await waitFor(() => expect(screen.getByText('Alice Johnson')).toBeInTheDocument());
    expect(screen.queryByText(/Pray through them one at a time/i)).not.toBeInTheDocument();
  });

  it('walks through the held people one at a time and marks each prayed', async () => {
    render(<PrayerList />);
    await waitFor(() => expect(screen.getByText('Alice Johnson')).toBeInTheDocument());

    fireEvent.click(screen.getAllByRole('button', { name: /Pray together/i })[0]);
    const session = await screen.findByTestId('pray-together-session');
    const inSession = within(session);

    // First card: Alice + her open prayer.
    expect(await within(session).findByText(/Praying together/i)).toBeInTheDocument();
    expect(inSession.getByText('Strength for finals')).toBeInTheDocument();
    fireEvent.click(inSession.getByRole('button', { name: /I prayed for Alice/i }));

    // Second card: Bob + his open prayer.
    expect(await within(session).findByText('Wisdom for the semester')).toBeInTheDocument();
    fireEvent.click(inSession.getByRole('button', { name: /I prayed for Bob/i }));

    // Recap.
    expect(await within(session).findByText(/You prayed for all 2/i)).toBeInTheDocument();
  });

  it('keeps an already-marked person checked when re-opened the same day', async () => {
    render(<PrayerList />);
    await waitFor(() => expect(screen.getByText('Alice Johnson')).toBeInTheDocument());

    fireEvent.click(screen.getAllByRole('button', { name: /Pray together/i })[0]);
    let session = await screen.findByTestId('pray-together-session');
    let inSession = within(session);
    await inSession.findByText(/Praying together/i);
    fireEvent.click(inSession.getByRole('button', { name: /I prayed for Alice/i }));
    fireEvent.click(inSession.getByRole('button', { name: /I prayed for Bob/i }));
    await inSession.findByText(/You prayed for all 2/i);
    fireEvent.click(inSession.getByRole('button', { name: /Close/i }));

    // Re-open the same day: Alice is already marked → her button reads "Prayed".
    fireEvent.click(screen.getAllByRole('button', { name: /Pray together/i })[0]);
    session = await screen.findByTestId('pray-together-session');
    inSession = within(session);
    await inSession.findByText(/Praying together/i);
    expect(inSession.getByText('Strength for finals')).toBeInTheDocument();
    expect(inSession.getAllByText(/Prayed/i).length).toBeGreaterThan(0);
  });

  it('skip advances without marking', async () => {
    render(<PrayerList />);
    await waitFor(() => expect(screen.getByText('Alice Johnson')).toBeInTheDocument());
    ...
    fireEvent.click(inSession.getByRole('button', { name: /Skip →/i }));
    expect(await inSession.findByText('Wisdom for the semester')).toBeInTheDocument();
    fireEvent.click(inSession.getByRole('button', { name: /Finish/i }));
    // Skipped both — nothing to close off.
    expect(await inSession.findByText(/Nothing to close off/i)).toBeInTheDocument();
  });

  it('closes the session on Escape', async () => {
    render(<PrayerList />);
    await waitFor(() => expect(screen.getByText('Alice Johnson')).toBeInTheDocument());

    fireEvent.click(screen.getAllByRole('button', { name: /Pray together/i })[0]);
    await screen.findByTestId('pray-together-session');

    fireEvent.keyDown(window, { key: 'Escape' });
    expect(screen.queryByTestId('pray-together-session')).not.toBeInTheDocument();
  });

  it('Go through again restarts the walk after the recap', async () => {
    render(<PrayerList />);
    await waitFor(() => expect(screen.getByText('Alice Johnson')).toBeInTheDocument());
    ...
    fireEvent.click(inSession.getByRole('button', { name: /Go through again/i }));
    session = await screen.findByTestId('pray-together-session');
    inSession = within(session);
    expect(await inSession.findByText(/Praying together/i)).toBeInTheDocument();
    expect(inSession.getByText('Strength for finals')).toBeInTheDocument();
  });
});
