...
//
// Deleting a conversation is a two-beat gesture: the row leaves the UI the
// moment the user taps Remove, but the Firestore deleteDoc (and its audit
// entry) only commit after a short Undo window. The pending commit lives here
// at module scope, so closing the contact modal or navigating away does not
// cancel it — only an explicit Undo does. An app kill inside the window leaves
// the interaction intact, which is the safe failure.
//
