import { doc, updateDoc, type Firestore, type WriteBatch, type DocumentReference } from 'firebase/firestore';
...
/**
 * Applies the derived activity patch onto a Firestore `WriteBatch`.
 */
export function applyContactActivityToBatch(
  batch: WriteBatch,
  contactRef: DocumentReference,
  stamp: ContactActivityStamp,
): void {
  const patch = buildContactActivityPatch(stamp);
  batch.update(contactRef, patch as Record<string, unknown>);
}
