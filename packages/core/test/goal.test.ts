...
/** An ISO string for the local day that `now` falls on, at the given time. */
const isoOn = (now: number, hour = 10, minute = 0) => {
...
describe('normalizeDayGoal', () => {
  ...
  it('keeps a valid on/count pair', () => {
    expect(normalizeDayGoal({ on: false, count: 7 })).toEqual({ on: false, count: 7 });
  });

  it('clamps count to the 1..20 band and rounds non-integers', () => {
    ...
    expect(normalizeDayGoal({ count: -3 })).toEqual({ on: true, count: GOAL_MIN });
    expect(normalizeDayGoal({ count: 2.6 })).toEqual({ on: true, count: 3 });
    expect(normalizeDayGoal({ count: '5' })).toEqual({ on: true, count: GOAL_DEFAULT_COUNT });
...
describe('goalCountFor', () => {
  ...
  it('counts only today’s people added by the uid', () => {
    const contacts = [
      ...
      contact({ createdBy: 'me', createdAt: daysAgo(now, 1) }), // yesterday — no
      contact({ createdBy: 'them', createdAt: isoOn(now, 10) }), // someone else — no
      contact({ createdBy: 'me' }), // no createdAt — no
    ];
    expect(goalCountFor(contacts, 'me', now)).toBe(2);
  });

  it('honours the addedBy fallback and the null uid', () => {
    ...
    expect(goalCountFor([contact({ createdBy: 'me', createdAt: isoOn(now) })], 'me')).toBeGreaterThanOrEqual(0);
  });
});

describe('goalNewToday', () => {
  it('counts everyone’s new people today, and only today', () => {
    const now = new Date('2026-08-25T12:00:00').getTime();
    const contacts = [
...
describe('goalMet / goalFill / goalShow', () => {
  const goal: DayGoal = { on: true, count: 5 };

  it('goalMet is only true once the count reaches the number', () => {
    expect(goalMet(4, goal)).toBe(false);
    expect(goalMet(5, goal)).toBe(true);
    expect(goalMet(6, goal)).toBe(true);
  });

  it('goalFill clamps to 0..1', () => {
    expect(goalFill(0, goal)).toBe(0);
    expect(goalFill(3, goal)).toBe(0.6);
    expect(goalFill(5, goal)).toBe(1);
    expect(goalFill(9, goal)).toBe(1);
  });

  it('goalShow gates on on, in-window and not-yet-met', () => {
    expect(goalShow(goal, 3, true)).toBe(true);
    expect(goalShow(goal, 5, true)).toBe(false); // met — the ring reverts
    expect(goalShow(goal, 3, false)).toBe(false); // outside the window
    expect(goalShow({ on: false, count: 5 }, 3, true)).toBe(false); // turned off
  });
});