// Shared pure helpers. NOTE: the web-only `cn()` (clsx + tailwind-merge) helper
// is intentionally NOT here — className merging is meaningless in React Native.
// Native styling uses the theme + StyleSheet / NativeWind instead.

export function sleep(ms: number) {
  // Reference the timer via globalThis so this stays platform-agnostic (no DOM
  // or node lib in @cisa/core's tsconfig) while working on web, node, and RN.
