...
//
// Storage: one team-wide doc `settings/partners` holding `{ byTerm: { "Fall
// 2026": [[uid, uid]] } }`. Admin-only writes; readable by the app so both
// sides of a pair and the creation paths can resolve "who goes out with me".
//
// The pure helpers take `byTerm`/groups explicitly so they're trivially
// testable; `applyPartners` additionally keeps a module-level view of the
// CURRENT term's groups so `addContact` can stamp `coCreators` synchronously
// without a Firestore read at creation time.
